...
import { AppBar, Avatar, Backdrop, Badge, Box, Drawer, IconButton, Stack, Toolbar, Tooltip, Typography } from '@mui/material'
...
const Header = ({ data, chatId }) => {
  ...
  useEffect(() => {
    ...
    const avatarSrc = user?.[0]?.avatar[0] || ""
    const userName = user?.[0]?.name || ""
    setOtherPersonAvatarSrc(avatarSrc)
    setOtherPerson(userName)
  }, [chatId])

  return (
    <>
      <Box sx={{ flexGrow: 1, height: { xs: "none", sm: "4rem" } }}>
        <AppBar
          position='static'
          sx={{
            bgcolor: "#ffffff",
